...
def calculate_bias_vec_balanced(data, blend_p, steepness, midpoint, convergence_strength):
    ...
    bias = np.minimum(bias, 0.5)  # Cap at 0.5
    return bias

# --- Initial Parameters ---
initial_blend_p = 0.6
initial_steepness = 10.0
initial_midpoint = 0.351
initial_convergence_strength = 0.5

# --- Create Figure and Axes ---
fig, ax = plt.subplots(figsize=(16, 12))
plt.subplots_adjust(bottom=0.25, right=0.85)  # Leave space for sliders and colorbar/legend

# Initial bias grid
initial_bias = calculate_bias_vec_balanced(
    grid_data, initial_blend_p, initial_steepness, initial_midpoint, initial_convergence_strength
)
bias_grid = initial_bias.values.reshape(100, 100)
im = ax.imshow(
    bias_grid, cmap="viridis", extent=[0, 1, 0, 1], origin='lower',
    aspect='auto', interpolation='bilinear', vmin=0, vmax=0.5
)
...
legend_elements.append(mlines.Line2D([], [], color='grey', marker='o', linestyle='None',
                                     markersize=7, label='Other Players'))
...
legend_elements.append(mlines.Line2D([], [], color='lime', marker='o', linestyle='None',
                                     markersize=10, label='Positive Label (Target)'))
legend_elements.append(mlines.Line2D([], [], color='red', marker='o', linestyle='None',
                                     markersize=10, label='Negative Label'))
ax.legend(handles=legend_elements, title="Legend", bbox_to_anchor=(1.04, 1), loc='upper left')
...
ax_blend = plt.axes([0.1, 0.20, 0.65, 0.03], facecolor=axcolor)
ax_steep = plt.axes([0.1, 0.15, 0.65, 0.03], facecolor=axcolor)
ax_mid = plt.axes([0.1, 0.10, 0.65, 0.03], facecolor=axcolor)
ax_conv = plt.axes([0.1, 0.05, 0.65, 0.03], facecolor=axcolor)
...
s_conv = Slider(ax_conv, 'Convergence Strength', 0.0, 1.0, valinit=initial_convergence_strength)
...
def update(val):
    ...
    new_bias = calculate_bias_vec_balanced(
        grid_data, blend_p, steepness, midpoint, convergence_strength
    )
...
# Adjust layout
fig.tight_layout(rect=[0, 0.25, 0.88, 1])  # Make space for sliders and legend
