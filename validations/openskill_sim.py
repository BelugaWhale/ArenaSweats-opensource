...
print(f"Using input: {default_path}\n")
...
baseline_rating_change = {
    pid: _simple_rating(baseline_after_ratings[pid].mu, baseline_after_ratings[pid].sigma)
...
# ----------------------------------------------------------------------
ub_rate_input, ub_reductions = check_for_unbalanced_lobby(
    model,
    teams_ratings,
    logger=None,
...
if ub_rate_input is None:
    ...
    ub_reductions = [0.0] * len(team_order_ids)
else:
...
for place, team in placing_with_team:
    ...
    if show_lobby and median_team_mu_value > 0.0:
        diff_pct = (team_mu_sum - median_team_mu_value) / median_team_mu_value
    else:
        diff_pct = 0.0

    is_unbalanced_team = show_lobby and median_team_mu_value > 0.0 and (diff_pct > 0.0)

    if is_unbalanced_team:
        scale = 1.0 - (diff_pct * UNBALANCED_TEAM_MU_REDUCTION)
        lobby_diff_str = f"{diff_pct * 100:.1f}% (scale: {scale * 100:.1f}%)"
    else:
        lobby_diff_str = ""
...
if _USE_RICH:
    table_ub = Table(
        title=f"UNBALANCED-LOBBY summary table (ordered by placing)\nmedian team mu: {median_team_mu_value:.2f}",
        show_lines=False,
    )
    for h in headers_ub:
    ...
    print("\nUNBALANCED-LOBBY summary table (ordered by placing)")
    print("=" * 220)
    print(f"median team mu: {median_team_mu_value:.2f}")
    print(
        "placing | player | pregame_player_stats | pregame_team_stats | "
        "postgame_team_stats | postgame_player_stats | rating_change | rating_change_diff | lobby_diff"
    )
    print("-" * 220)
    for r in rows_ub:
        print(
            f"{r[0]:7} | {r[1]:42} | {r[2]:26} | {r[3]:26} | {r[4]:26} | "
            f"{r[5]:26} | {r[6]:>13} | {r[7]:>18} | {r[8]:>21}"
        )
